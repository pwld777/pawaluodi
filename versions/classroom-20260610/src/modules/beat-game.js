...
import { playInstrument, unlockAudio, warmHtmlAudioFallback } from "./audio-engine.js";
    ...
    <section class="game-layout beat-layout enter-view">
      ...
      <div class="stage-card drum-stage">
        ...
        <div class="huagu" role="group" aria-label="可点击花鼓">
          <img class="huagu-image" src="./assets/images/flower-drum-tablet.png?v=qq-safe-25" alt="红色大花鼓，鼓面朝上，鼓身贴有花纹" loading="eager" fetchpriority="high" decoding="sync">
...
export function bindBeatGame({ root, state, setState, render, onReward }) {
  ...
  addSafeTapListener(root.querySelector("[data-beat-switch]"), () => {
    stopTicker();
    setState({
      ...state.get(),
      beatGame: {
  ...
  addSafeTapListener(root.querySelector("[data-start-beat]"), async () => {
    await unlockAudio();
    stopTicker();
  ...
  addSafeTapListener(root.querySelector("[data-stop-beat]"), () => {
    stopTicker();
    announceFeedback(feedback, "已经停止，可以等教师口令再开始。");
  });

  addSafeTapListener(root.querySelector("[data-reset-beat]"), () => {
    stopTicker();
    setState({
      ...state.get(),
      beatGame: {
  ...
  root.querySelectorAll("[data-drum-surface]").forEach((button) => {
    ...
    let lastTouchHitAt = 0;
    const handleDrumHit = async (event) => {
      const now = performance.now();
      if ((event.type === "click" || event.type === "touchend") && now - lastPointerHitAt < 320) {
        return;
      }
      if (event.type === "click" && now - lastTouchHitAt < 420) {
        return;
      }
      ...
      warmHtmlAudioFallback("hand-drum", { accent: isCenterHit });
    ...
    button.addEventListener("touchend", handleDrumHit, { passive: false });
  ...
  addSafeTapListener(root.querySelector("[data-switch-now]"), () => {
    if (!session) {
