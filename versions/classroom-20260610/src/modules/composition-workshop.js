...
import { playInstrument, preloadInstrument, unlockAudio, warmAudioWithRecordedSample, warmHtmlAudioFallback } from "./audio-engine.js";
import { announceFeedback } from "./feedback.js";
import { renderRhythmMark } from "./rhythm-mark.js";
import { addSafeTapListener } from "./safe-tap.js";
    ...
    <div class="compose-instrument-row" aria-label="选择乐器">
      ${compositionInstruments.map((instrument) => `
        <button class="${composition.instrument === instrument.id ? "active" : ""}" data-instrument="${instrument.id}" type="button" style="--instrument:${instrument.color}" aria-pressed="${composition.instrument === instrument.id ? "true" : "false"}">
          <span class="instrument-photo" style="background-image:url('${instrument.image}'); background-position:${instrument.imagePosition}" aria-hidden="true">
            <em class="instrument-symbol">${instrument.symbol}</em>
          </span>
...
function shouldUseTabletTapToPlace() {
...
function suppressFollowupClick(element, delay = 220) {
    ...
    <section class="compose-game-shell tablet-compose-layout ${composition.isComplete ? "compose-is-complete" : ""} enter-view">
      <div class="compose-arcade-stage compose-quiet-stage">
        <div class="compose-stage-canvas" data-compose-game-stage aria-label="小乐队闯关台动画舞台">
          <img class="compose-stage-fallback" src="./assets/images/compose-stage-scene.jpg?v=qq-safe-25" alt="小乐队节奏闯关舞台">
        </div>
...
export function bindCompositionWorkshop({ root, state, setState, render, onReward }) {
  ...
  function placeBlockFromTap(block) {
    const current = state.get();
    const barIndex = current.composition.activeBarIndex ?? 0;
    void unlockAudio()
      .then(() => preloadInstrument(current.composition.instrument))
      .catch(() => {});
    updateComposition({
      root,
      state,
  ...
  root.querySelectorAll("[data-instrument]").forEach((button) => {
    addSafeTapListener(button, () => {
      ...
      warmHtmlAudioFallback(instrumentId);
      unlockAudio()
  ...
  root.querySelectorAll("[data-block-id]").forEach((block) => {
    block.addEventListener("pointerdown", (event) => {
      if (shouldUseTabletTapToPlace()) {
        event.preventDefault();
        suppressFollowupClick(block, 180);
        placeBlockFromTap(block);
        return;
      }
    ...
    addSafeTapListener(block, () => {
      if (block.dataset.skipClick) {
        return;
      }

      if (shouldUseTabletTapToPlace()) {
        suppressFollowupClick(block);
        placeBlockFromTap(block);
        return;
      }
  ...
  addSafeTapListener(root.querySelector("[data-clear-composition]"), () => {
    clearPlayback();
    const current = state.get();
    setState({
      ...current,
  ...
  addSafeTapListener(root.querySelector("[data-play-composition]"), async () => {
    clearPlayback();
    const current = state.get();
    warmHtmlAudioFallback(current.composition.instrument);
    ...
    try {
      ...
      // iPad Safari can reject a decode attempt and still allow immediate playback retry.
    }
