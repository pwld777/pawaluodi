...
| 节奏拖拽 4 道题 | `src/modules/rhythm-drag-game.js` + `src/data/rhythm-questions.js` | `npm run verify` 覆盖 4 道题答案；浏览器烟测覆盖重做后第 1 题推进到第 2 题 |
| 创编工坊 2 小节创编 | `src/modules/composition-workshop.js` + `src/modules/game-logic.js` | `npm run verify` 覆盖小节容量、超出拒绝、3/4 附点二分音符完成 |
| 至少 4 个音色可播放 | `src/data/instrument-sounds.js` + `src/modules/audio-engine.js` | `npm run verify` 覆盖 4 个乐器 id；浏览器烟测覆盖 4 个按钮选中态 |
